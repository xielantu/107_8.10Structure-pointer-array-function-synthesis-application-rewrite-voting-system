// 107_8.10结构体指针数组函数综合应用改写选票系统.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
struct XuanMin* initXms(struct XuanMin* p,int* pn)//初始化选民信息//这个int* pn只提供地址，经过这个函数的调用后，赋给pn一个数值
{
	int i;
	if (p == NULL)
	{
